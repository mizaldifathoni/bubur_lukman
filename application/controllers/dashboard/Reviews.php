...
class Reviews extends CI_Controller
{
		...
		if($this->ModelLogin->isAccessable()){
			if(null !== $this->input->post('inputLogout')){
				...
				$data = array(
					...
					'menu_reviews'	=> $this->getMenuReviewsHtml()
				);
  ...
  private function getShopReviewsHtml() {
	...
	private function getMenuReviewsHtml() {
		$reviews = $this->ModelUlasanMenu->getAllReviews();
		...
		if($reviews == null){
			...
			$overallRatings = $this->ModelUlasanMenu->getOverallMenuRatings();
			$reviewsCount = count($reviews);
			...
			foreach($reviews as $review) {
				$isPositiveReview = ($review->rating_menu_toko >= 3);

				$html .= '
								<div class="col-lg-12 mb-4 ' . (($isPositiveReview)? 'menupositive' : 'menunegative') . '">
                  <div class="card shadow-sm rounded">
                    <div class="card-body align-middle d-flex h-almost-100">
                      <div class="row justify-content-center align-self-center w-100 m-1">
                        <div class="col-lg-2 d-flex justify-content-center align-items-center">
                            ...
                            <i class="fa fa-user fa-5x"></i>
												...
												<div class="col-lg-10 d-flex flex-column align-items-start pr-0">
													<div class="d-inline-block mb-2">
														<strong>' . $review->nama_pengulas . '</strong><small class="text-muted"> — mengulas ' . $review->nama_menu . ' pada ' . $review->nama_toko . '</small>
													</div>
                          <p class="mb-3">' . $review->isi_ulasan_menu_toko . '</p>
                          <div class="row w-100 d-flex justify-content-between pl-3">
                            <div class="col-xs-6">
				';

				for($i=1; $i<=$review->rating_menu_toko; $i++){
					$html .= '
															<i class="fa fa-star text-primary mb-1"></i>
					';
				}

				for($i=$review->rating_menu_toko+1; $i<=5; $i++){
					$html .= '
															<i class="fa fa-star text-muted mb-1"></i>
					';
				}
				...
				$html .= '
                            ...
                            <div class="col-xs-6">
                              <small class="mb-1 text-muted">' . $this->Time->relativeTime(strtotime($review->tanggal_ulasan_menu_toko)) . '</small>
                            </div>
                          </div>
                        </div>
                      </div>
